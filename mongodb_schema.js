...
// ==============================================================================
// MongoDB Schema for Mindmap Online Real-time System (using Mongoose)
// Version: 1.0
...
// ==============================================================================
/**
 * Main schema for storing mindmap documents
...
const MindmapSchema = new Schema({
  ...
  category: {
    type: String,
    enum: ['work', 'personal', 'education', 'project', 'brainstorming', 'other'],
    default: 'other'
  },
  isFavorite: {
    type: Boolean,
...
mongoose.connect('mongodb://localhost:27017/mindmap_system', {
...
console.log('Mindmap created:', newMindmap._id);
...
// 4. Track a change in history
const { MindmapHistory } = require('./mongodb_schema');

const historyEntry = new MindmapHistory({
  mindmapId: mindmap._id,
  mysqlUserId: 1,
  action: 'node_add',
  changes: {
    nodeId: 'node-2',
    data: { text: 'New branch' }
  }
});
await historyEntry.save();


// 5. Create a realtime session
const { RealtimeSession } = require('./mongodb_schema');
...
// 6. Query mindmaps by user
const userMindmaps = await Mindmap.find({
  $or: [
    { mysqlUserId: userId },
...
// 7. Search mindmaps
const searchResults = await Mindmap.find(
  { $text: { $search: 'project planning' } },
