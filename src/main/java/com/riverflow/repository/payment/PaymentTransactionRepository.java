...
public interface PaymentTransactionRepository extends JpaRepository<PaymentTransaction, Long>,
     ...
     */
    @Query("SELECT COALESCE(SUM(p.transferAmount), 0) FROM PaymentTransaction p " +
            "WHERE p.status = 'processed' AND p.createdAt BETWEEN :startDate AND :endDate")
     ...
     */
    @Query("SELECT COALESCE(SUM(p.transferAmount), 0) FROM PaymentTransaction p " +
            "WHERE p.status = 'processed' AND p.transferType = 'in'")
     ...
     */
    @Query("SELECT COALESCE(SUM(p.transferAmount), 0) FROM PaymentTransaction p " +
            "WHERE p.status = 'processed' AND p.transferType = 'in' AND p.createdAt >= :startDate")
