...
public class AiMindmapServiceImpl implements AiMindmapService {
    ...
    public MindmapResponse generateMindmap(GenerateMindmapRequest request, Long userId) {
        ...
        String mode = (reqMode == null || reqMode.isBlank() || "default".equalsIgnoreCase(reqMode)) ? "normal"
                : reqMode;
        ...
        Map<String, Object> payload = buildGeminiPayloadForGenerate(topic, levels, firstLevelCount, lang,
                request.getTags(), mode, minFirst, maxFirst);
        ...
        if (roots.isEmpty())
            throw new InvalidMindmapDataException("root", "Không có root node");
        ...
        if (level1Count < minFirst || level1Count > maxFirst) {
            throw new InvalidMindmapDataException("level1",
                    String.format("Số node cấp 1 phải %d–%d", minFirst, maxFirst));
        }
    ...
    public MindmapResponse optimize(OptimizeRequest request, Long userId) {
        ...
        boolean isEditor = mindmap.getCollaborators() != null && mindmap.getCollaborators().stream()
                .anyMatch(c -> Objects.equals(c.getMysqlUserId(), userId) && "accepted".equals(c.getStatus())
        ...
        String lang = request.getLanguage() != null ? request.getLanguage() : "vi";
        // Removed: extractAgentPlanFromHints - frontend no longer sends AGENT_PLAN
        // Backend AI will analyze user request dynamically instead
        // Map<String, Object> agentPlan =
        // extractAgentPlanFromHints(request.getHints());
        // if (agentPlan != null) {
        // String planMsg = "Agent Plan: " + agentPlan.getOrDefault("summary",
        // agentPlan);
        ...
        List<String> labels = mindmap.getNodes().stream()
                ...
                .limit(50)
                .collect(Collectors.toList());
        Map<String, Object> agentPayload = buildGeminiPayloadForClassifyAction(title, desc, labels, lang,
                request.getHints());
        ...
        if (StringUtils.hasText(nodeLabel) && !StringUtils.hasText(request.getNodeId())) {
            ...
            if (StringUtils.hasText(foundId))
                request.setNodeId(foundId);
        }
        ...
        agentLogs.add("Agent Analyze: target=" + request.getTargetType()
                + (StringUtils.hasText(request.getNodeId()) ? ", nodeId=" + request.getNodeId() : "")
                + (StringUtils.hasText(request.getStructureType()) ? ", structureType=" + request.getStructureType()
                        : ""));
            ...
            for (Map<String, Object> op : agentOps) {
                ...
                if (type.equals("delete_node") || type.equals("delete_subtree")) {
                    ...
                    if (StringUtils.hasText(id)) {
                        ...
                        mindmap.setNodes(
                                mindmap.getNodes().stream().filter(n -> !toRemove.contains(String.valueOf(n.get("id"))))
                                        .collect(Collectors.toList()));
                    ...
                    if (StringUtils.hasText(id) && StringUtils.hasText(newLabel)) {
                        for (Map<String, Object> n : mindmap.getNodes()) {
                            if (id.equals(String.valueOf(n.get("id")))) {
                                Map<String, Object> data = (Map<String, Object>) n.getOrDefault("data",
                                        new HashMap<>());
                    ...
                    if (StringUtils.hasText(label)) {
                        String parentId = StringUtils.hasText(parentLabel) ? findNodeIdByLabel(mindmap, parentLabel)
                                : null;
                        if (!StringUtils.hasText(parentId)) {
                            Set<String> targets = mindmap.getEdges().stream().map(e -> String.valueOf(e.get("target")))
                                    .collect(Collectors.toSet());
                            Optional<Map<String, Object>> rootNodeOpt = mindmap.getNodes().stream()
                                    .filter(n -> !targets.contains(String.valueOf(n.get("id")))).findFirst();
                            if (rootNodeOpt.isPresent())
                                parentId = String.valueOf(rootNodeOpt.get().get("id"));
                            else if (!mindmap.getNodes().isEmpty())
                                parentId = String.valueOf(mindmap.getNodes().get(0).get("id"));
                        }
                        String newId = newNodeId();
                        Map<String, Object> data = new HashMap<>();
                        data.put("label", label);
                        ...
                        opsChangedGraph = true;
                        agentLogs.add("Added node: " + label
                                + (StringUtils.hasText(parentLabel) ? " under " + parentLabel : ""));
                        broadcastAgentLog(mindmap.getId(), agentLogs.get(agentLogs.size() - 1));
                    }
                    ...
                    if (StringUtils.hasText(sId) && StringUtils.hasText(tId)) {
                        boolean exists = mindmap.getEdges().stream()
                                .anyMatch(e -> Objects.equals(String.valueOf(e.get("source")), sId)
                                        && Objects.equals(String.valueOf(e.get("target")), tId));
                        if (!exists) {
                            Map<String, Object> edge = new HashMap<>();
                            edge.put("id", newEdgeId());
                            edge.put("source", sId);
        ...
        if ("node".equalsIgnoreCase(target)) {
            ...
            if (request.getHints() != null) {
                for (String h : request.getHints()) {
                    ...
                    if (t.matches(".*(xóa|xoá|remove|delete|bỏ|loại bỏ|drop).*"))
                        wantsDelete = true;
                    if (t.matches(".*(nhánh|branch|subtree|con|hậu duệ).*"))
                        wantsPruneSubtree = true;
                }
            }
            ...
            Map<String, Object> payload = buildGeminiPayloadForOptimizeNode(currentLabel, siblingLabels, lang,
                    request.getHints());
            String json = callGemini(payload);
            JsonNode root = parseJson(json);
            ...
            Map<String, Object> payload = buildGeminiPayloadForOptimizeDescription(mapTitle, currentDesc, lang,
                    request.getHints(), "normal");
            String json = callGemini(payload);
            JsonNode root = parseJson(json);
            ...
            UpdateMindmapRequest.UpdateMindmapRequestBuilder builder = UpdateMindmapRequest.builder()
                    .description(newDesc);
            if (opsChangedGraph) {
            ...
            boolean isReplace = agentPlan != null
                    && "replace".equalsIgnoreCase(String.valueOf(agentPlan.get("action")));
            List<String> pruneLabels = agentPlan != null && agentPlan.get("pruneLabels") instanceof List<?> pl
                    ? pl.stream().map(String::valueOf).collect(Collectors.toList())
            ...
            if (isReplace) {
                String topic = (request.getHints() != null && !request.getHints().isEmpty()) ? request.getHints().get(0)
                        : anchorLabel;
                Map<String, Object> payloadGen = buildGeminiPayloadForGenerate(topic, levels, firstLevelCount, lang,
                        null, mode, minFirst, maxFirst);
                json = callGemini(payloadGen);
            } else {
                Map<String, Object> payload = buildGeminiPayloadForExpandStructure(anchorLabel, existingChildren, lang,
                        request.getHints(), mode, minFirst, maxFirst, firstLevelCount, levels, structureType);
            ...
            for (JsonNode n : nodesNode) {
                ...
                int lvl = (parentTempId == null || parentTempId.isBlank()) ? 1
                        : (nodeLevel.getOrDefault(parentTempId, 1) + 1);
                nodeLevel.put(tempId, lvl);
                levelIndex.put(tempId, levelIndex.getOrDefault(parentTempId == null ? "__root__" : parentTempId, 0));
                levelIndex.put(parentTempId == null ? "__root__" : parentTempId,
                        levelIndex.getOrDefault(parentTempId == null ? "__root__" : parentTempId, 0) + 1);
            }
            ...
            int topLevelCount = (int) parentByTempId.entrySet().stream()
                    .filter(e -> e.getValue() == null || e.getValue().isBlank()).count();
            ...
            for (JsonNode n : nodesNode) {
                ...
                data.put("label", label);
                // simple diversity
                String[] shapes = new String[] { "rectangle", "circle", "diamond", "hexagon", "ellipse",
                        "roundedRectangle" };
                ...
                if (parentTempId == null || parentTempId.isBlank()) {
                    ...
                    idx = (int) parentByTempId.entrySet().stream()
                            .filter(e -> e.getValue() == null || e.getValue().isBlank()).map(Map.Entry::getKey)
                            .collect(Collectors.toList()).indexOf(tempId);
                    if (idx < 0)
                        idx = 0;
                    ...
                    Optional<Map<String, Object>> parentNodeOpt = rfNodes.stream()
                            .filter(nn -> String.valueOf(nn.get("id")).equals(parentNewId)).findFirst();
                    ...
                    int childIdx = (int) parentByTempId.entrySet().stream()
                            .filter(e -> Objects.equals(e.getValue(), parentTempId)).map(Map.Entry::getKey)
                            .collect(Collectors.toList()).indexOf(tempId);
                    if (childIdx < 0)
                        childIdx = 0;
            ...
            MindmapResponse updated = mindmapService.updateMindmap(mindmap.getId(), updateReq, userId);
            appendAgentLogs(updated, agentLogs);
            for (String logMsg : agentLogs)
                broadcastAgentLog(mindmap.getId(), logMsg);
            return updated;
        } else {
    ...
    // int words = Arrays.stream(label.trim().split("\\s+")).filter(s ->
    // !s.isBlank()).toArray().length;
    // if (words < 1 || words > 4) {
    ...
    private String callGemini(Map<String, Object> payload) {
        try {
                    ...
                    .exchangeToMono(clientResponse -> {
                        if (clientResponse.statusCode().is2xxSuccessful()) {
                            ...
                            return clientResponse.bodyToMono(String.class).map(body -> {
                                ...
                                throw new com.riverflow.exception.AiUpstreamException(code,
                                        parseOpenAiError(body, code));
                            });
                        }
                    })
                    .block();
            ...
            if (candidates == null || candidates.isEmpty())
                throw new IllegalStateException("No candidates from Gemini");
            Map<?, ?> c0 = (Map<?, ?>) candidates.get(0);
            Map<?, ?> content = (Map<?, ?>) c0.get("content");
            if (content == null)
                throw new IllegalStateException("No content in candidate");
    ...
    private Map<String, Object> buildGeminiPayloadForGenerate(String topic, int levels, int firstLevelCount,
            String lang, List<String> tags, String mode, int minFirst, int maxFirst) {
        ...
        String user = String.format(
                "Tạo mindmap về chủ đề: '%s' (ngôn ngữ: %s). Độ sâu: %d. Số node cấp 1: %d (giới hạn %d–%d theo MODE: %s).\n"
                        +
                        "Trả về JSON dạng:\n" +
    ...
    private Map<String, Object> buildGeminiPayloadForOptimizeNode(String currentLabel, List<String> siblingLabels,
            String lang, List<String> hints) {
    ...
    private Map<String, Object> buildGeminiPayloadForOptimizeDescription(String title, String currentDesc, String lang,
            List<String> hints, String mode) {
        ...
        if (StringUtils.hasText(currentDesc))
            user.append("Mô tả hiện tại: ").append(currentDesc).append("\n");
        if (hints != null && !hints.isEmpty()) {
            user.append("Gợi ý: ").append(String.join(", ", hints)).append("\n");
        }
    ...
    private Map<String, Object> buildGeminiPayloadForClassifyAction(String title, String currentDesc,
            List<String> nodeLabels, String lang, List<String> hints) {
        StringBuilder user = new StringBuilder();
        user.append("Bạn là hệ thống phân tích yêu cầu người dùng cho mindmap hiện tại.\n");
        user.append("Tiêu đề: ").append(title).append("\n");
        if (StringUtils.hasText(currentDesc))
            user.append("Mô tả: ").append(currentDesc).append("\n");
    ...
    private Map<String, Object> buildGeminiPayloadForExpandStructure(String anchorLabel, List<String> existingChildren,
            String lang, List<String> hints, String mode, int minFirst, int maxFirst, int firstLevelCount, int levels,
            String structureType) {
        StringBuilder user = new StringBuilder();
        user.append("Mở rộng mindmap cho nhánh: ");
        user.append(anchorLabel != null ? anchorLabel : "ROOT");
        user.append(". Ngôn ngữ: ").append(lang).append(". ");
        user.append("Độ sâu: ").append(levels).append(". Số node cấp 1: ").append(firstLevelCount).append(" (giới hạn ")
                .append(minFirst).append("–").append(maxFirst).append(")").append(".\n");
        ...
        user.append(
                "Trả JSON: { \"nodes\": [ {\"id\": \"n1\", \"label\": \"...\", \"parentId\": null }, {\"id\": \"n2\", \"label\": \"...\", \"parentId\": \"n1\" } ] }\n");
        Map<String, Object> userContent = Map.of(
                "role", "user",
                "parts", List.of(Map.of("text", user.toString())));
        Map<String, Object> generationConfig = new HashMap<>();
        ...
        try {
            ...
            for (String h : hints) {
                if (h != null && h.startsWith("AGENT_PLAN:")) {
                    ...
                    List<String> pruneLabels = plan.get("pruneIds") instanceof List<?> pl ? null
                            : (plan.get("pruneLabels") instanceof List<?> p2 ? (List<String>) (List<?>) p2
                                    : Collections.emptyList());
            ...
            String action = joined.matches(".*(sửa lại|đổi|thay|chuyển sang|thành|replace|change to|update to).*")
                    ? "replace"
    ...
    private void broadcastAgentLog(String mindmapId, String text) {
        try {
            if (!StringUtils.hasText(realtimeServerUrl))
                return;
